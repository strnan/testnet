...
4. Inisialisasi Working Directory
    ...
    - Menggunakan Snapshot `(Optional)`
        ...
        Hapus `genesis.json` sebelumnya dan download file `genesis.json`
...
6. Jalankan Node
    ...
    Pada bagian ini kalian harus menunggu download headers ke 100% dan kemudian baru melakukan sinkroninsasi block. _Jangan sampai close tab sebelum itu selesai, maka nanti akan terjadi error_. Jadi kalau kalian ingin tutup tab, pastikan stop dulu dengan cara `CTRL + C` agar tidak terjadi error saat menjalankan service nanti.
